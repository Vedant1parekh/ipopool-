-- When A clubs a PAN onto B's application, the same relationship now forms
-- automatically in the other direction too: B's own applied PAN (the one
-- behind the application A just clubbed onto) gets clubbed onto A's own
-- application that uses the exact PAN A clubbed in with - so B never has
-- to manually redo the same action back.
--
-- The reciprocal insert needs profile_id = B while the caller is A, which
-- the existing "auth.uid() = profile_id" insert policy can't allow from a
-- plain client query. So the whole club-in (both directions) moves into
-- one SECURITY DEFINER function that re-derives and re-validates
-- everything itself from auth.uid() and the two inputs - it can't be
-- abused by calling it directly, since nothing about the reciprocal side
-- is trusted from caller input.

create function public.club_in_with_reciprocal(p_application_id uuid, p_pan_card_id uuid)
returns void
language plpgsql
...
begin
  ...
  -- already used that card against the caller, or already hit their own
  -- per-owner cap.
...
grant execute on function public.club_in_with_reciprocal(uuid, uuid) to authenticated;
